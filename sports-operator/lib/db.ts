...
// Single local SQLite database for this personal application.
// The path can be swapped later (e.g. to point at a Supabase/Postgres
// adapter) without touching call sites, since all access goes through
...
declare global {
  ...
  var __sportsOperatorDb: Database.Database | undefined;
}

function getDb(): Database.Database {
  if (!global.__sportsOperatorDb) {
    const db = new Database(resolvedPath);
    db.pragma('journal_mode = WAL');
    db.pragma('foreign_keys = ON');
    migrate(db);
    global.__sportsOperatorDb = db;
  }
  return global.__sportsOperatorDb;
}
...
export function listEntries(): EntryRecord[] {
  const db = getDb();
  const rows = db.prepare('SELECT * FROM entries ORDER BY created_at DESC').all();
  return rows.map(rowToEntry);
}

export function createEntry(entry: EntryRecord): EntryRecord {
  const db = getDb();
  db.prepare(
...
export function updateEntryStatus(
  ...
  db.prepare(
    `UPDATE entries SET status = ?, settled_at = ?, profit_loss = ? WHERE id = ?`
  ).run(status, new Date().toISOString(), profitLoss, id);
